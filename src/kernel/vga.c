...
void
VGA_putpixel (uint32_t x, uint32_t y, uint32_t color)
{
  switch (bpp)
    {
    case 8:
...
void
VGA_drawrect (uint32_t x, uint32_t y, uint32_t width, uint32_t height,
	      uint32_t color)
{
...
void
VGA_drawchar (uint32_t x, uint32_t y, uint32_t c, uint32_t fg_color,
	      uint32_t bg_color)
{
...
void
VGA_print (uint32_t x, uint32_t y, uint8_t *s, uint32_t fg_color,
	   uint32_t bg_color)
{
  unsigned int i = 0;
  while (s[i])
...
uint32_t
VGA_getpixel (uint32_t x, uint32_t y)
{
  uint32_t *pixel = (uint32_t*) (fb + pitch * y + (bpp / 8) * x);
  return *pixel;
}
